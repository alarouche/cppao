...
void active::atomic_fifo::push(atomic_node*n)
{
	...
	while( !input_queue.compare_exchange_weak( i, n ) );
}
...
active::atomic_node * active::atomic_fifo::pop()
{
	...
	for(;;)
	{
		...
		{
			if( output_queue.compare_exchange_weak( t, &busy ) )
			{
		...
		{
			atomic_node * old_output = output_queue.exchange( &busy );
...
active::atomic_node * active::atomic_lifo::pop()
{
	...
	atomic_node * l;
	do {
		l=list.load(); // std::memory_order_acquire);
		if( l==&busy &&--spin_count==0 )
		{
			std::this_thread::sleep_for(std::chrono::seconds(0));
			spin_count=2000;
		}
	} while ( l==&busy || !list.compare_exchange_weak( l, &busy ) );
